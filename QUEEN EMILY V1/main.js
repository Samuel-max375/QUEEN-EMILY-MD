...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (text === 'autosend' || text === 'save') {
            await autosend(msg, chatId); // No sock here
        }
    });
}
...
async function handleGroupParticipantUpdate(sock, update) {
    try {
        ...
        if (action === 'add') {
            ...
            const channelId = welcomeData?.channelId || '0029Vb77pP4A89Mje20udJ32@newsletter';
        ...
        if (action === 'remove') {
            ...
            const channelId = goodbyeData?.channelId || '0029Vb77pP4A89Mje20udJ32@newsletter';
