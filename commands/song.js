...
async function songCommand(sock, chatId, message) {
    try {
        ...
        let additionalInfo = '';
        if (videoInfo) {
            const uploadDate = videoInfo.uploadDate ? new Date(videoInfo.uploadDate) : null;
            const year = uploadDate ? uploadDate.getFullYear() : (videoInfo.ago ? new Date().getFullYear() : 'Unknown year');
            const views = videoInfo.views ? videoInfo.views.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : 'Unknown';
            const artist = videoInfo.author ? videoInfo.author.name : 'Unknown artist';
            
            additionalInfo = `🎵 *Title:* ${videoInfo.title || result.result.title}\n` +
                           `🎤 *Artist:* ${artist}\n` +
                           `👀 *Views:* ${views}\n` +
                           `📅 *Year:* ${year}\n` +
                           `⏱ *Duration:* ${videoInfo.timestamp || result.result.duration || 'Unknown'}\n\n` +
                           `_Downloading your song..._\n` +
                           `*QUEEN EMILY MD* \n` +
                           `> 𝐏𝐎𝐖𝐄𝐑𝐄𝐃 𝐁𝐘 𝐂.𝐎 𝐓𝐄𝐂𝐇`;
        } else {
            additionalInfo = `🎵 *Title:* ${result.result.title}\n\n` +
                           `_Downloading your song..._\n` +
                           `*QUEEN EMILY MD* \n` +
                           `> 𝐏𝐎𝐖𝐄𝐑𝐄𝐃 𝐁𝐘 𝐂.𝐎 𝐓𝐄𝐂𝐇`;
        }
