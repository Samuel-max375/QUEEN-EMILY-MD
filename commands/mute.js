...
async function muteCommand(sock, chatId, senderId, durationInMinutes) {
    ...
    if (!isBotAdmin) {
        await sock.sendMessage(chatId, { text: ' make QUEEN EMILY an admin first.' });
        return;
    }

    if (!isSenderAdmin) {
        await sock.sendMessage(chatId, { text: 'Are you stupid ??.  Only group admins can use the mute command.' });
        return;
    }
    ...
    try {
        ...
        setTimeout(async () => {
            ...
            await sock.sendMessage(chatId, { text: 'The group has been unmuted peasants. * QUEEN EMILY MD is awsome* ' });
