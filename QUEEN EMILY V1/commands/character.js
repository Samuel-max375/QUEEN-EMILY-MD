...
async function characterCommand(sock, chatId, message) {
    ...
    if (!userToAnalyze) {
        await sock.sendMessage(chatId, { 
            text: 'Please mention someone or reply to their message for QUEEN EMILY MD to analyze their character!', 
            ...channelInfo 
        });
        return;
    }
    ...
    try {
        ...
        const analysis = `🔮 *QUEEN EMILY MD Character Analysis* 🔮\n\n` +
            `👤 *User:* ${userToAnalyze.split('@')[0]}\n\n` +
            `✨ *Key Traits:*\n${traitPercentages.join('\n')}\n\n` +
            `🎯 *Overall Rating:* ${Math.floor(Math.random() * 21) + 80}%\n\n` +
            `👑 Powered by *QUEEN EMILY MD* — for fun only!`;
        ...
        await sock.sendMessage(chatId, { 
            text: 'QUEEN EMILY MD could not analyze character! Try again later.',
            ...channelInfo 
        });
    }
}
